...
and the diffusion coefficient \(\sigma:\mathbb{R}^d\to\mathbb{R}^{d\times d}\) is state-dependent and non-constant. The main differnnce from
the ordianry Stochastic Diferential Equation is the dependence on the all other particles via push-forward measure \( \mu_t \), that why
"with the interaction" comes.
...
In \Cref{sec:jacobian-bound} we investigate the norm of Jacobian and how its affect the functional inequalities.
...
In \Cref{sec:averaged-jacobian}, we replace the pathwise supremum over a compact
set by an averaged Jacobian energy and derive a corresponding moment estimate.
...
matrix in \cref{eq:JacSDE} and the norm \(\|J_t(u)\|\) on the compact sets.
...
\end{equation*}
We now state the SDE satisfied by \(J_t(u)\) for \cref{eq:main_eqn} from \cite{dorogovtsev2023intermittency}.
...
measure \( \mu_t \), the Jacobian SDE in \cref{eq:JacSDE} does not contain the
derivative with respect to it. Because of that fact we can evaluate
\(\|J_t(u)\|\) using Kunita's theorem for the stochastic flows.

Under \Cref{ass:general_lipschitz}, standard results on stochastic flows (see, e.g., \cite[Theorem~1.4.1]{Kunita1986}) yield a modification such that for a.s.\ \(\omega\) and each \(t\ge0\) the map \(u\mapsto T_t(\omega,u)\) is \(\mathcal{C}^1\).
...
\begin{assumption}\label{ass:spatial-lip-derivatives}
    There exist constants \(L^{(2)}<\infty\) and \(L_\sigma^{(2)}<\infty\)
    such that for all \(\mu\in\mathcal{P}_2(\mathbb{R}^d)\) and all
    \(x,y\in\mathbb{R}^d\),
    \begin{align*}
        \|\partial_x a(x,\mu)-\partial_x a(y,\mu)\|_{\mathrm{op}}
         & \le L^{(2)}\,|x-y|,        \\
        \max_{1\le k\le d}\|\partial \sigma^{(k)}(x)-\partial \sigma^{(k)}(y)\|_{\mathrm{op}}
         & \le L_\sigma^{(2)}\,|x-y|.
    \end{align*}
\end{assumption}

\begin{proposition}[Moment bound for \(\mathcal{K}_t(\cdot;K)\)]\label{prop:Kt-moment}
    Assume \Cref{ass:general_lipschitz,ass:spatial-lip-derivatives}.
    Then for every compact \(K\subset\mathbb{R}^d\), every \(p\ge 1\),
    and every \(t\ge0\),
    ...
    (depending only on \(p,K,d,L,L_\sigma,L^{(2)},L_\sigma^{(2)}\)) such that
