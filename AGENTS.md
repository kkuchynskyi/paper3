...
- In displayed math environments, keep each logical formula row as horizontally compact as possible: put the full equation, inequality, or estimate on one physical source line whenever it fits within 147 columns.  Break displayed math only when the row would exceed 147 columns or when there is a genuine mathematical row break; for real multiline derivations, use `align` or `align*` with one logical equation row per physical source line.
